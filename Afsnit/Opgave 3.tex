...
\[
\varepsilon_M^{(\theta=0)} = -0.0900,
\qquad
\varepsilon_M^{(\theta=0.05)} = -0.0911,
\]
so the difference is $\Delta \varepsilon_M = -0.0011 $, meaning that childcare costs makes the Marshall elasticity slightly more negative. Thereby labour supply becomes marginally more responsive, but the effect is quantitatively small in this calibration.
